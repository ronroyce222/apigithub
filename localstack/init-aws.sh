...
awslocal sns create-topic --name jira-events --region $REGION

# Create SQS queues
echo "Creating SQS queues..."
awslocal sqs create-queue --queue-name github-events-queue --region $REGION
awslocal sqs create-queue --queue-name jira-events-queue --region $REGION
...
awslocal sqs create-queue --queue-name jira-events-dlq --region $REGION
...
JIRA_QUEUE_ARN=$(awslocal sqs get-queue-attributes \
    --queue-url http://localhost:4566/000000000000/jira-events-queue \
    --attribute-names QueueArn \
    --query 'Attributes.QueueArn' \
    --output text \
    --region $REGION)
...
awslocal sns subscribe \
    --topic-arn arn:aws:sns:us-east-1:000000000000:jira-events \
    --protocol sqs \
    --notification-endpoint $JIRA_QUEUE_ARN \
    --region $REGION
