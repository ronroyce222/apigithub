...
class TestJiraWebhookErrors:
    """Tests for Jira webhook error handling."""

    @pytest.mark.asyncio
    async def test_jira_webhook_publish_returns_none(
        self,
        async_client: AsyncClient,
        jira_secret: str,
    ) -> None:
        ...
        with patch("routers.jira.SNSPublisher") as mock_publisher_class:
            mock_publisher = AsyncMock()
            mock_publisher.publish_jira_event = AsyncMock(return_value=None)
            mock_publisher_class.return_value = mock_publisher
    ...
    @pytest.mark.asyncio
    async def test_jira_webhook_publish_exception(
        self,
        async_client: AsyncClient,
        jira_secret: str,
    ) -> None:
        """Test webhook returns 500 when publish raises exception."""
        payload = {"webhookEvent": "jira:issue_created"}
        body = json.dumps(payload).encode("utf-8")
        signature = create_signature(body, jira_secret)
        ...
        with patch("routers.jira.SNSPublisher") as mock_publisher_class:
            mock_publisher = AsyncMock()
            mock_publisher.publish_jira_event = AsyncMock(
                side_effect=RuntimeError("Unexpected error")
            )
            mock_publisher_class.return_value = mock_publisher
