...
@pytest.mark.asyncio
async def test_publish_jira_event_returns_message_id(
    settings: Settings,
    mock_sns: AsyncMock,
) -> None:
    """Test publishing Jira event returns message ID."""
    publisher = SNSPublisher(settings)
...
@pytest.mark.asyncio
async def test_publish_jira_event_includes_attributes(
    settings: Settings,
    mock_sns: AsyncMock,
) -> None:
    """Test Jira event includes correct message attributes."""
    publisher = SNSPublisher(settings)
    ...
    with patch.object(
        ...
        await publisher.publish_jira_event("jira:issue_created", {"test": "d"})

    call_kwargs = mock_sns.publish.call_args.kwargs
    assert call_kwargs["MessageAttributes"]["source"]["StringValue"] == "jira"
    assert call_kwargs["MessageAttributes"]["event_type"]["StringValue"] == \
        "jira:issue_created"
